...
const checks = [
  ...
  // used follows the freshest STEP's input (beadaf0), not the turn slice:
  // in this fake the step input is the conversation's cumulative input, so
  // used must equal usage.total.inputTokens, never last.totalTokens.
  ["meter/unknown-model-has-no-window", t1usage.every((u) => u.modelContextWindow === null) && t1windows.length === 2 && t1windows.every((w, i) => w.size === null && w.snapshot === undefined && w.used === t1usage[i].total.inputTokens + t1usage[i].total.cachedInputTokens), JSON.stringify(t1windows.map((w) => [w.used, w.size]))],
  ["meter/gemini-window-estimated-1m", t2usage.length === 1 && t2usage[0].modelContextWindow === 1000000 && t2windows.length === 1 && t2windows[0].used === t2usage[0].total.inputTokens + t2usage[0].total.cachedInputTokens && t2windows[0].size === 1000000 && t2windows[0].snapshot?.estimated === true && t2windows[0].snapshot?.contextWindowTokens === 1000000, JSON.stringify(t2windows.map((w) => [w.used, w.size, w.snapshot?.estimated]))],
