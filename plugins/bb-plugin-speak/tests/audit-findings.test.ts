/**
 * AUDIT (PLUG-34) — failing tests that demonstrate defects found by reading.
 *
 * These are DELIBERATELY FAILING at HEAD. They document behaviour the audit
 * considers wrong; they are not a spec the code currently meets. Delete or
 * move each one into the real suites once the defect it pins is fixed.
 *
 * The browser stubbing is a trimmed copy of tests/player.test.ts's harness.
 *
