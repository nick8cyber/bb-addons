// bb-plugin-sidebar-collapse — a BB plugin backend entry.
//
...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb sidebar-collapse` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config sidebar-collapse`. Add `secret: true` for values like API keys.
  ...
  // The `bb sidebar-collapse` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb sidebar-collapse list [--json]",
    "  bb sidebar-collapse add <title> [--json]",
    "  bb sidebar-collapse done <todo-id> [--json]",
    "  bb sidebar-collapse undo <todo-id> [--json]",
    "  bb sidebar-collapse remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    ...
    summary: "Manage the Sidebar Collapse plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb sidebar-collapse list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb sidebar-collapse add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb sidebar-collapse done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb sidebar-collapse undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb sidebar-collapse remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb sidebar-collapse list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
