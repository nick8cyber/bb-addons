...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb sidebar-collapse` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb sidebar-collapse`. BB imports it into agent threads automatically.
...
`bb sidebar-collapse add "Ship it"` in a terminal. The page updates at once.
...
bb plugin config sidebar-collapse set showDone false
bb plugin reload sidebar-collapse
```
